...
=============================================
RSI, MA, EMA     — denní svíčky (200 dní)
Demand/Supply zóny — logika:
  1. Na týdenních svíčkách najdi 2 poslední swing lows (= místa odkud přišel silný pohyb nahoru)
  2. Pro každý swing low týden jdi na denní svíčky
  3. Najdi poslední bearish denní svíčku PŘED prvním silným bullish dnem
  4. zone_low  = low  té bearish svíčky
  5. zone_high = high té bearish svíčky
  Supply zóny — symetricky (poslední bearish svíčka před silným pohybem dolů)
"""
...
def _parse_ohlcv(raw: list[dict]) -> list[dict]:
    """Převede raw TwelveData záznamy, vrátí chronologicky vzestupně."""
    parsed = []
...
def _resample_weekly(candles: list[dict]) -> list[dict]:
    """Resampleuje denní OHLCV na týdenní. Vstup chronologicky vzestupně."""
...
def compute_technical(raw_ohlcv: list[dict], current_price: float) -> dict:
    ...
    RSI + MA z denních svíček.
    Demand/Supply zóny z týdenního TF zpřesněné denními svíčkami.
    """
